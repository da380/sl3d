program adj_test
  ...
  ! Forward calculation
  call forward_full_sl(lmax,nt,dt,nt,ice_data,sea_level_data,sl_fin)
  ...
  call stat_adj(lmax,ice_load,sl_fin(:,:,nt),adj_load,UVp_adj(:,:,1),Q_adj(:,:,1),K_adj(1))


  ps = 0.0_dp
  Us = 0.0_dp
  call surface_fields(lmax,UVp_adj(:,:,1),ps,Us)

  call adj_sl_ice(lmax,ice_load,sl_fin(:,:,nt),adj_load,Us,ps,Q_adj(:,:,1),K_adj(1),sl_adj,ice_adj)

  call kern_ice_it(lmax,ice_load,sl_fin(:,:,nt), &
  ...
  do it = 2,nt
     ...
     call update_full_rk_adj(lmax,dt,ice_load,sl_fin(:,:,nt-it+2),mem(:,:,:,:,:,:,1), &
                             UVp_adj(:,:,1),W_adj(:,:,1),mem(:,:,:,:,:,:,2),UVp_adj(:,:,2), &
                             W_adj(:,:,2),Q_adj(:,:,1),K_adj(1),Q_adj(:,:,2),K_adj(2))

     ps = 0.0_dp
     Us = 0.0_dp
     call surface_fields(lmax,UVp_adj(:,:,2),ps,Us)
     ...
     call adj_sl_ice(lmax,ice_load,sl_fin(:,:,nt-it+1),adj_load,Us,ps,Q_adj(:,:,2),K_adj(2),sl_adj,ice_adj)

     ! Calculate kernel
     call kern_ice_it(lmax,ice_load,sl_fin(:,:,nt-it+1),Us,ps, &
