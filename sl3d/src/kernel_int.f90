program kernel_int
  ...
  integer(i4b) :: ilayer,ispec,inode,igl,iphi,int1,int2
  ...
  open(newunit=io,file='gracevk2264.12',action='read')
  do ilayer = 1,nsect
     ...
     do ispec = sect_ind(1,ilayer),sect_ind(2,ilayer)
        do inode = 1,ngll
           ...
           do iphi = 1,nphi
              do igl = 1,ngl
                 read(io,*) real1,real1,real2,real1,real2,kern
           ...
           call zero_coef_from_fun(spat,sint)
           sint = sint*sqrt(fourpi_d)*rr**2
           integral = integral + wgll(inode)*jac_element(ispec)*real(sint)
        end do
     end do
  end do
  close(io)

  print *, integral*pot_norm
